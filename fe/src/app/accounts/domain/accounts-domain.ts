...
export class AccountsDomain {
  ...
  private _accounts = new Signal<Account[]>([]);
  ...
  get accountsBroadcast() {
    return this._accounts.broadcast;
  }
  ...
  createAccount = async (input: { name: string; type: AccountType }) => {
    await this.createAccountRunner.execute(async () => {
      ...
      this._accounts.set(accounts);
      this.selectedAccountId.set(createdAccountExists ? createdAccount.id : (accounts[0]?.id ?? null));
      this.message.set(
